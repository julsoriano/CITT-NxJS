...
function ChartistPage(props, dataChart) {
...
export const getServerSideProps = async (context) => {
  console.log("In setServerSideProps")
  const res = await fetch(`https://localhost:5000/api/owner/getnoofcomps`)
  ...
  const dataChart = ((data) => {
    ...
    series: Array(data.map((data1) => {
      return data1.noOfComponents;
    }))
  })

  /*
  const labels = data.map((data1) => 
    data1.desc
  )

  const series = data.map((data1) => 
    data1.noOfComponents
  )

  const dataChart = ({labels: labels, series: Array(series)})
  */
