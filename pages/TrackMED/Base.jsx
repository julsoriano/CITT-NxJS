...
// className={classNames(Styles.table, Styles.table-striped, Styles.table-condensed, Styles.table-hover)} 
...
export class Base extends Component {
    ...
    */
    renderCommonTable(descriptions) { 
        return <table className={classNames(Styles.table, Styles.tablestriped, Styles.tablecondensed, Styles.tablehover)}  cellSpacing='0' width='100%'>
            <thead>
                <tr role='row'>
                    <th></th>
                    <th>Index</th>
            ...
            {descriptions.map( (description, index) =>
                <tr key={ index + 1 }>
                    <td data-id={ description.id } className={classNames(Styles.glyphicon, Styles.glyphiconplus)}  
                        onClick={this.showRelatedTable}></td>
                    {/*<td><Icon onClick={this.showRelatedTable}>Add</Icon></td> */}
                    <td>{ index + 1 }</td>
    ...
    popupComponentTable(root) {
        ReactDOM.render(  
        <td colSpan = '7'>
            <table className={classNames(Styles.table,  Styles.tablelight, Styles.tablestriped)}>
				<thead>
					<tr>
						<th>Index</th>
						<th>Asset#</th>
    ...
    */
    ...
    createNTableHTML(id, elGP, elP, headings) {
        ...
        .then(data => {
            ...
            tbl.classList.add('table', 'tablelight', 'tablestriped', 'tablecondensed');
    ...
    showRelatedTable(event) {
        ...
        // Find if elList includes 'glyphiconplus'
        const iterator = elList.values();
        var evtDOMTokenList = "";
        for (let value of iterator) {
            console.log("${value} " + `${value}`);
            if (`${value}`.includes('glyphiconplus')) {
                evtDOMTokenList = `${value}`
                break;
            }
        }
        console.log(id);
        ...
        div.appendChild(span);
        */
        // if (elList.contains('glyphiconplus')) {
        if (evtDOMTokenList.includes('glyphiconplus')) {
            
            elList.replace('glyphiconplus', 'glyphiconminus');
            
            if( this.tr !== null && this.tr !== undefined) this.tr.parentNode.removeChild(this.tr);
            if( this.elListSave !== null && this.elListSave !== undefined ) this.elListSave.replace('glyphiconminus', 'glyphiconplus');
            ...
            elList.replace('glyphiconminus', 'glyphiconplus');
